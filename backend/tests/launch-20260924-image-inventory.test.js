...
it('a batch leltár felismeri a metaadatot, lapozható és nem változtatja meg a DB-t', async () => {
  ...
  const first = await scanPublicImages({ db, readImage, kind: 'avatar', limit: 1 });
  ...
  expect((await scanPublicImages({ db, readImage, kind: 'avatar', after: first.next_cursor })).scanned).toBe(0);
  expect((await db.query('SELECT * FROM users WHERE id=$1', [owner.id])).rows[0]).toEqual(before);
  expect((await scanPublicImages({ db, readImage, kind: 'avatar', limit: 1 })).results).toEqual(first.results);
});
...
it('a bizonyítékként hivatkozott listing eredetije külön kézi ellenőrzésre kerül', async () => {
  ...
  const result = await scanPublicImages({ db, readImage, kind: 'listing' });
