...
//  - a SOFŐR lép vissza → a fuvar díjmentesen újranyílik, a feladó a korábbi
...
test('sofőr-visszalépés: díjmentes újraválasztás a korábbi ajánlatok közül', async ({ page }) => {
  const shipper = await createUser('shipper', 'Feladó Ferenc');
  const carrier = await createUser('carrier', 'Sofőr Sándor');
  const backup = await createUser('carrier', 'Tartalék Tibor');
  const job = await createJob(shipper);
  // Két licit, majd az első sofőr elfogadva + díj fizetve
  ...
  // A feladó sofőrt cserél (a sofőr nem elérhető)
  await loginAs(page, shipper);
  await page.goto(`/dashboard/fuvar/${job.id}`);
  await page.getByRole('button', { name: /Másik sofőrt választok/ }).click();
  ...
  // A tartalék sofőr elfogadása — új fizetés NEM kell, a kontakt azonnal jön
  await page.getByRole('button', { name: /^Elfogadom/ }).first().click();
  await expect(page.getByText(/A SOFŐR ELÉRHETŐSÉGE/).first()).toBeVisible({ timeout: 20_000 });
